...
_RESPONSE_MAP = {
    ...
    "DB_CONNECTION_POOL_EXHAUSTED": (503, "Database connection pool is exhausted. All connections are currently in use."),
...
def get_db2_error_response(sqlstate: str = None, sqlcode: int = None, default_message: str = None) -> tuple[int, str]:
    """
...
class ResponseCode:
    ...
    logs this result
    '''
    def __init__(self, error_tag: str = "", data: Optional[Any] = None, sqlstate: str = None, sqlcode: int = None):
        '''
        Args:
        ...
        if sqlstate or sqlcode is not None:
            self.__error_code, self.__message = get_db2_error_response(sqlstate, sqlcode)
        ...
        if(not self.__success):
            self.__logger.error(f"{self.__error_code}. {self.__error_tag}: {self.__message}\n\t\t\tdata: {self.__data}")
        else:
            self.__logger.info(f"{self.__error_code}. {self.__error_tag}: {self.__message}\n\t\t\tdata: {self.__data}")
