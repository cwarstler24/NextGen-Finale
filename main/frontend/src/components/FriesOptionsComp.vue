...
<template>
    ...
    <div name="options" class="option-grid">
        ...
        <select :value="props.modelValue[0]" @change="updateOption(0, $event.target.value)">
            ...
            {{ opt }}<span v-if="props.productOptions.option1Price[optIdx] > 0"> ${{ props.productOptions.option1Price[optIdx].toFixed(2) }}</span>
            </option>
        </select>
        ...
        <select :value="props.modelValue[1]" @change="updateOption(1, $event.target.value)">
            ...
            {{ opt }}<span v-if="props.productOptions.option2Price[optIdx] > 0"> + ${{ props.productOptions.option2Price[optIdx].toFixed(2) }}</span>
            </option>
        </select>
        <label>{{productOptions.optionNames[2]}}</label>
        <div class="checkbox-group" name="option3">
            <label
              v-for="(opt, optIdx) in props.productOptions.option3"
              :key="opt"
              class="checkbox-option"
            >
                <input
                  type="checkbox"
                  :checked="getOption3Selections().includes(opt)"
                  @change="toggleOption3(opt, $event.target.checked)"
                />
                <span>
                    {{ opt }}<span v-if="props.productOptions.option3Price[optIdx] > 0"> + ${{ props.productOptions.option3Price[optIdx].toFixed(2) }}</span>
                </span>
            </label>
        </div>
    </div>
